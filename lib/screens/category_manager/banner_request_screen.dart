...
/// Review screen for one category_ads banner request — preview, who submitted
/// it, the category's ad-subscription standing, and Approve/Reject. Approve
/// publishes the banner live on the category's public page immediately (see
/// category_ads/review.php).
class BannerRequestScreen extends StatefulWidget {
  final int categoryAdId;

  const BannerRequestScreen({super.key, required this.categoryAdId});
...
class _BannerRequestScreenState extends State<BannerRequestScreen> {
  ...
  Future<void> _load() async {
    try {
      final ads = await _categoryAdService.mine();
      final ad = ads.firstWhere((a) => a.id == widget.categoryAdId);
      final category = await _categoryService.get(ad.categoryId!);
      if (!mounted) return;
      setState(() {
        _ad = ad;
        _category = category;
  ...
  Future<void> _submitReview(String status, {String? note}) async {
    ...
    try {
      await _categoryAdService.review(widget.categoryAdId, status, note: note);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    if (ad == null || category == null) {
      return const Scaffold(body: Center(child: CircularProgressIndicator()));
    }

    final isPending = ad.status == 'pending';
    final subscriptionActive = category.hasActiveAdSubscription;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _InfoRow(label: 'Category', value: category.name),
              ...
              Expanded(
                child: Text(
                  category.subscriptionExpiresAt == null
                      ...
                      : subscriptionActive
                          ? 'Active until ${DateFormat.yMMMd().format(category.subscriptionExpiresAt!)}'
                          : 'Expired on ${DateFormat.yMMMd().format(category.subscriptionExpiresAt!)}',
                ),
              ),
            ],
          ),

...
$user = current_user();
require_role($user, ['super_admin', 'category_manager', 'category_staff']);
...
// A category manager or category staff may only edit their own linked category.
if (!is_super_admin($user)) {
    if ($user['category_id'] === null || (int) $user['category_id'] !== (int) $_POST['id']) {
        json_error('Forbidden: this is not your category', 403);
    }
}
...
// The featured embed is a curation task delegated to category_staff, reviewed
...
if ($user['role'] === 'category_staff' || is_super_admin($user)) {
    $embedHosts = [
...
// A super_admin's own edit stays live immediately — a category is only ever
...
// reasoning as services/update.php.
if (!is_super_admin($user)) {
...
// force an expiry) — on top of the category manager's own self-service Razorpay
// purchase via payments/create_order.php's 'category_subscription' case.
...
if (is_super_admin($user)) {
    if (isset($_POST['extend_days']) && $_POST['extend_days'] !== '') {
        category_extend_subscription($pdo, (int) $_POST['id'], (int) $_POST['extend_days']);
        ...
        $pdo->prepare('UPDATE categories SET subscription_expires_at = ? WHERE id = ?')->execute([$value, $_POST['id']]);
    }
}

if (is_super_admin($user)) {
    log_admin_action($user, 'category.update', 'category', $_POST['id'], [

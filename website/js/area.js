...
function categoryCardHtml(category) {
  return `
    <a href="/sehy_web/category.html?id=${category.id}&area=${encodeURIComponent(areaName)}"
      ...
      <div class="p-5 flex-1 flex flex-col justify-between space-y-3">
        <div>
          ...
          ${category.description ? `<p class="text-xs text-slate-500 mt-1">${escapeHtml(category.description)}</p>` : ''}
        </div>
  ...
  if (categories.length === 0) {
    resultsEl.innerHTML = `
      <div class="text-center py-16">
        ...
        <p class="text-sm text-slate-500 mt-1">We're still adding categories.</p>
      </div>`;
    return;
  }
  resultsEl.innerHTML = `

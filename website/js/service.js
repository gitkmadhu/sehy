...
/** Footer policy links — external URL wins when set, otherwise an inline expandable block of the free-text version. Renders nothing if the service has none of these set. */
function policiesFooterHtml(service) {
  const items = [];
  if (service.terms_url) {
    items.push(`<a href="${escapeHtml(service.terms_url)}" target="_blank" rel="noopener">Terms &amp; Conditions</a>`);
  } else if (service.terms_text) {
    items.push(`<details><summary>Terms &amp; Conditions</summary><p>${escapeHtml(service.terms_text)}</p></details>`);
  }
  if (service.privacy_url) {
    items.push(`<a href="${escapeHtml(service.privacy_url)}" target="_blank" rel="noopener">Privacy Policy</a>`);
  } else if (service.privacy_text) {
    items.push(`<details><summary>Privacy Policy</summary><p>${escapeHtml(service.privacy_text)}</p></details>`);
  }
  if (service.refund_text) {
    items.push(`<details><summary>Refunds &amp; Returns</summary><p>${escapeHtml(service.refund_text)}</p></details>`);
  }
  if (service.shipping_text) {
    items.push(`<details><summary>Shipping &amp; Pickup</summary><p>${escapeHtml(service.shipping_text)}</p></details>`);
  }
...
/**
 * Bottom-sheet with deep links to the service's real Google listing — no
 * rating or review text is fetched or displayed here, since that would
 * require a paid Places API integration this service hasn't set up.
 */
function reviewsModalHtml(service) {
  const placeId = encodeURIComponent(service.google_place_id);
  return `
    <div class="reviews-modal" id="reviews-modal">
      <div class="reviews-sheet">
        <h3>Google Reviews</h3>
        <p>See what shoppers are saying about ${escapeHtml(service.name)} on Google, or leave your own review.</p>
...
async function init() {
  ...
  try {
    const [{ service }, { offers }, { ads }, { products }] = await Promise.all([
      api.get('/services/get.php', { id: serviceId }),
      api.get('/offers/list.php', { service_id: serviceId }),
      api.get('/service_ads/list.php', { service_id: serviceId }),
      api.get('/service_products/list.php', { service_id: serviceId }),
    ]);
    document.title = `${service.name} - Sehy`;
    trackServiceView(service.id, service.category_id, service.area);

    const hasEmbeds = !!(
      youtubeVideoId(service.embed_youtube_url) ||
      service.embed_instagram_url ||
      service.embed_facebook_url ||
      service.embed_twitter_url
    );

    const actions = [];
    if (service.whatsapp)
      actions.push(
        `<a class="btn outline" href="https://wa.me/${escapeHtml(service.whatsapp)}" target="_blank" rel="noopener">WhatsApp</a>`
      );
    if (service.website)
      actions.push(
        `<a class="btn outline" href="${escapeHtml(service.website)}" target="_blank" rel="noopener">Website</a>`
      );
    ...
    content.innerHTML = `
      ...
      ${service.tag_name || actions.length ? `
      <div class="service-identity">
        ${service.tag_name ? `<div class="service-tag">${escapeHtml(service.tag_name)}</div>` : ''}
      ...
      <div class="social-icon-row">${socialEmbedIconLinks(service).join('')}</div>
      <div id="featured-embeds"></div>` : ''}

      ${service.description ? `
      <div class="about-card">
        <div class="about-label">About ${escapeHtml(service.name)}</div>
        ${escapeHtml(service.description)}
      </div>` : ''}

      ${service.google_place_id ? `
      <div class="cta-row">
      ...
      ${policiesFooterHtml(service)}
    `;

    if (service.google_place_id) {
      content.insertAdjacentHTML('beforeend', reviewsModalHtml(service));
    ...
    document.getElementById('wa-subscribe-btn').addEventListener('click', async () => {
      ...
      btn.disabled = true;
      try {
        await api.post('/service_whatsapp/subscribe.php', { service_id: service.id });
    ...
    grid.innerHTML = offers.length
      ...
      : '<div class="empty-state">No active offers from this service yet</div>';
  } catch (e) {
